...
interface ProfilePhotoSelectorProps {
  image: string;
  formdata: initialRegisterDataType;
  setProfilePic: React.Dispatch<React.SetStateAction<initialRegisterDataType>>  ;
}

  
const ProfilePhotoSelector = ({ image, setProfilePic , formdata }: ProfilePhotoSelectorProps) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const [previewUrl, setpreviewUrl] = useState("");

const hanndleimageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  const file = e.target.files?.[0];
  if (file) {
    const preview = URL.createObjectURL(file);
    setProfilePic((prev) => ({ ...prev, profileImageUrl: preview }));
...
const handleRemoveImage = () => {
  setProfilePic((prev) => ({ ...prev, profileImageUrl: "" }));
  setpreviewUrl("");
};
